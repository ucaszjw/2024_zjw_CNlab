...
int tcp_sock_write(struct tcp_sock *tsk, char *buf, int len)
{
	...
	while (remain_len) {
		...
		packet_len = ETHER_HDR_SIZE + IP_BASE_HDR_SIZE + TCP_BASE_HDR_SIZE + send_len;
		char *packet = (char *)malloc(packet_len);
		memcpy(packet + ETHER_HDR_SIZE + IP_BASE_HDR_SIZE + TCP_BASE_HDR_SIZE, buf + handle_len, send_len);
...
int tcp_retrans_send_buf(struct tcp_sock *tsk)
{
	...
	log(DEBUG, "retransmit packet, seq: %u\n", ntohl(tcp->seq));
	ip_send_packet(packet, send_buffer_entry->len);
...
int tcp_recv_ofo_buf_add_packet(struct tcp_sock *tsk, struct tcp_cb *cb)
{
	...
	recv_ofo_buf_entry->len = cb->pl_len;
	recv_ofo_buf_entry->data = (char *)malloc(cb->pl_len);
	memcpy(recv_ofo_buf_entry->data, cb->payload, cb->pl_len);
