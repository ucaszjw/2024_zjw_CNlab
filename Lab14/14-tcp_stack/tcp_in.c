...
static inline void tcp_update_window(struct tcp_sock *tsk, struct tcp_cb *cb)
{
	...
	if(old_snd_wnd!=tsk->snd_wnd)
	{
		//将当前时间和新wdn写入cwdn.txt文件
		struct timespec time;
		long long timenow;
		clock_gettime(CLOCK_MONOTONIC, &time);
		timenow = time.tv_sec * 1000000000 + time.tv_nsec;
		FILE *file = fopen("cwnd.txt", "a");
		fprintf(file, "Current Time: %lldns, New Window Size: %d\n", timenow, tsk->snd_wnd);
		fclose(file);
	}
}
		...
		case TCP_LISTEN: 
			if (cb->flags == TCP_SYN) {
				...
				log(DEBUG, "Pass " IP_FMT ":%hu -> " IP_FMT ":%hu from process to listen_queue", 
						HOST_IP_FMT_STR(ctsk->sk_sip), ctsk->sk_sport,
						HOST_IP_FMT_STR(ctsk->sk_dip), ctsk->sk_dport);
